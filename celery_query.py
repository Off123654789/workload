...
def simple_query(sparql_query,spatial_query=0):
    ...
    scheduler_address = 'tcp://192.168.0.50:8786'
    ...
    # 使用Ray并行执行查询和过滤
    futures = [client.submit(execute_query_and_filter,machine, query,workers = machine[0]) for machine, query in queries.items()]
    completed_futures, not_completed_futures = wait(futures)

    # 获取任务的结果
    all_results = [future.result() for future in completed_futures]
    client.close()
